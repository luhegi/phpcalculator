...
class Utility extends Command {
    ...
    protected function operation(InputInterface $input, OutputInterface $output, $operation) {
        ...
        $textoutput = $text ." = ";
        $this->addHistory($operation, $text, $result, $textoutput . $result);

        $output->writeln($textoutput . $result);
    }

    protected function addHistory($operation, $description, $result, $output)
    { 
        $json = new \stdClass();
